...
namespace Task6.Hubs
{
    public class GameHub : Hub
    {
        ...
        private static string Key(Guid sessionId, string playerName)
            => $"{sessionId:N}|{(playerName ?? string.Empty).Trim().ToLowerInvariant()}";
        ...
        public async Task Subscribe(Guid sessionId, string playerName)
        {
            ...
            if (_pendingDisconnects.TryRemove(key, out var cts))
            {
                ...
                await _hub.Clients.Group(groupName).SendAsync("OpponentReconnected", new { playerName });
            }
        ...
        public async Task LeaveSession(Guid sessionId)
        {
            ...
            var playerName = (info.playerName ?? string.Empty).Trim();
            if (string.IsNullOrWhiteSpace(playerName))
                throw new HubException("Invalid username");
            ...
            try
            {
                shouldDelete = _service.LeaveSession(sessionId, playerName);
            }
            catch
            {
                shouldDelete = false;
            ...
            await _hub.Clients.Group(group).SendAsync("OpponentLeft", new { playerName });

            if (!shouldDelete)
            {
                SessionStateDto state = null;
                try
                {
                    state = _service.GetSessionState(sessionId);
                }
                catch
                {
        ...
        public override Task OnDisconnectedAsync(Exception? exception)
        {
            if (_connections.TryRemove(Context.ConnectionId, out var info))
            {
                ...
                if (!string.IsNullOrWhiteSpace(playerName))
                {
                    ...
                    _ = _hub.Clients.Group(group).SendAsync("OpponentDisconnected", new { playerName, seconds = GRACE_SECONDS });
                    ...
                    _ = Task.Run(async () =>
                    {
                        try
                        {
                            ...
                            if (_pendingDisconnects.TryRemove(key, out var myCts))
                            {
                                ...
                                try
                                {
                                    delete = _service.LeaveSession(sessionId, playerName);
                                }
                                catch
                                {
